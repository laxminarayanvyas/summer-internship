...
public class ExcelHelper {
    ...
    public static ByteArrayInputStream OPFileToCSV(List<OutputFile> files) {
    	...
    	String[] HEADERs = { "DATE", "TIME(UTC)", "NO. DEALERS", "SIDE",
        ...
        try (ByteArrayOutputStream out = new ByteArrayOutputStream();
            ...
            DateTimeFormatter outputFormatter = DateTimeFormatter.ofPattern("dd-MM-yyyy"); // Change the date format as per your requirement
            ...
            for (OutputFile file : files) {
                String[] rowData = {
                		...
                		//String.format("%04d-%02d-%02d", file.getTrade_date().getYear(), file.getTrade_date().getMonthValue(), file.getTrade_date().getDayOfMonth()),
                       //file.getTrade_date().format(outputFormatter),
                        ...
                        file.getMaturity().format(outputFormatter),
 ...
 //public static ByteArrayInputStream consolidatedOP(List<ConsolidatedOPFile> files) {
    public static void consolidatedOP(List<ConsolidatedOPFile> dataList, Writer writer) throws IOException {
    	
    	String[] HEADERs = {
    		    "output_file_dtl_id", "process_guid", "clubbed_trades_count", "trade_date", "trade_time",
    		    "dealers_in_competition", "side", "isin", "ticker", "maturity", "coupon_perc",
    		    "size_in_MM_actual", "size_in_MM", "currency", "price_actual", "price", "mid_price_actual",
    		    "mid_price", "yield_perc", "spread", "settlement_date", "on_venue", "venue_actual",
    		    "venue_mic", "venue_identifier", "process_trade", "auto_execution", "portfolio_trade",
    		    "buy_side", "buy_side_lei", "client_unique_key", "counter_party", "counter_party_lei",
    		    "counterparty_identifier", "secmst_entity_name", "secmst_ticker", "secmst_sector",
    		    "secmst_country", "secmst_issue_date", "secmst_maturity_date", "secmst_issue_currency",
        ...
        //try (//ByteArrayOutputStream out = new ByteArrayOutputStream();
            ...
            for (ConsolidatedOPFile file : dataList) {
                String[] rowData = {
                	    ...
                	    file.getTrade_date().format(formatter),
                	    file.getTrade_time().format(timeFormatter),
                	    file.getDealers_in_competition() == null ? "" : file.getDealers_in_competition().toString(),
                	    file.getSide() == null ? "" : file.getSide(),
                	    file.getIsin() == null ? "" : file.getIsin(),
