...
public class ExcelHelper {
	...
	public static Stream<String> OPFileToCSV(List<OutputFile> files) {
		...
		Stream<String> csvLines = files.stream().map(file -> {
			String[] rowData = { file.getTrade_date().format(DateTimeFormatter.ofPattern("yyyy-MM-dd")),
					...
					file.getMaturity() == null ? ""
							: file.getMaturity().format(DateTimeFormatter.ofPattern("yyyy-MM-dd")),
					...
					file.getSettlement_date().format(DateTimeFormatter.ofPattern("yyyy-MM-dd")),
	...
	public static Stream<String> consolidatedOP(List<ConsolidatedOPFile> dataList) throws IOException {
		...
		Stream<String> csvLines = dataList.stream().map(file -> {
			String[] rowData = { file.getOutput_file_dtl_id().toString(),
	...
	public static byte[] generateCSVData(List<ConsolidatedOPFile> dataList) throws IOException {
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd"); // Change the date format as per your
																					// requirement
		DateTimeFormatter timeFormatter = DateTimeFormatter.ofPattern("HH:mm:ss");

		StringBuilder csvContent = new StringBuilder(getHeaderRow());
		...
		for (ConsolidatedOPFile file : dataList) {
			...
			csvContent.append(String.join(",", rowData));
			csvContent.append(System.lineSeparator());
		}
	...
	private static String getHeaderRow() {
		String[] HEADERs = { "output_file_dtl_id", "process_guid", "clubbed_trades_count", "trade_date", "trade_time",
				"dealers_in_competition", "side", "isin", "ticker", "maturity", "coupon_perc", "size_in_MM", "currency",
				"price_actual", "price", "mid_price_actual", "mid_price", "yield_perc", "spread", "settlement_date",
				"on_venue", "venue_actual", "venue_mic", "venue_identifier", "process_trade", "auto_execution",
				"portfolio_trade", "buy_side", "buy_side_lei", "client_unique_key", "counter_party",
				"counter_party_lei", "counterparty_identifier", "secmst_entity_name", "secmst_ticker", "secmst_sector",
